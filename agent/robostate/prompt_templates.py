...
8. PROGRESSION & GOAL NODES (CRITICAL): Only the final sink nodes of the SDG (nodes with no outgoing edges) represent the ultimate goal state. Other intermediate nodes (like an item originally being on a sofa) are merely prerequisites. Do NOT attempt to maintain prerequisite states if they conflict with progressing toward the final goal (e.g., if you must hold an item to move it, do not put it back just because an intermediate node says it was on a sofa).
